...
![image](https://github.com/user-attachments/assets/74007d21-f327-499e-80a9-496da128df12)
...
```
관계 : 결합하려는 데이터 간의 유형이 동일해야 함. (필드가 고유 값이 아니면 데이터가 중복해서 들어갈 수 있음.)
조인 : 제어 혹은 의도적인 필터링, 복제를 위해.
데이터 중복 및 필터링 문제가 생길 수 있음.
...
inner조인 : 데이터를 결합할 경우 두 테이블 모두의 일치하는 항목만을 포함. 테이블 간 데이터가 일치하지 않으면 값이 완전히 삭제됨.
Left, Right조인 : 한쪽 테이블의 모든 값과 반대쪽 테이블에 해당하는 일치 항목만을 포함함. 일치하는 데이터가 없을 경우 데이터가 null로 표시됨.
Outer조인 : 두 페이블의 모든 값을 완전히 포함하는 테이블이 생성됨. 한 테이블의 값이 다른 테이블에 일치하는 항목이 없는 경우에도 데이터가 null로 표시됨.
...
![image](https://github.com/user-attachments/assets/98d02411-30bf-4e42-847d-b9fc41de7d74)
